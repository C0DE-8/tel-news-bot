...
async function handleAdminRoute(req, res, controller, requireHttpAdmin) {
  ...
  try {
    if (req.method === "GET" && pathname === "/admin/news-config") {
      ...
      sendJson(res, 200, {
        ok: true,
        groups: await controller.listConfigs(),
      });
      return;
    }
    ...
    if (req.method === "GET" && pathname.startsWith("/admin/news-config/")) {
      ...
      const group = await controller.getConfig(chatId);
    ...
    if (req.method === "POST" && pathname === "/admin/news-stop") {
      const payload = await readJsonBody(req);
      requireHttpAdmin(req, payload);
      const group = await controller.stopNews(payload.chatId);
      sendJson(res, 200, { ok: true, group });
      return;
    }
    ...
    if (req.method === "POST" && pathname === "/admin/news-start") {
      const payload = await readJsonBody(req);
      requireHttpAdmin(req, payload);
      const group = await controller.startNews(payload.chatId);
      sendJson(res, 200, { ok: true, group });
      return;
    }
